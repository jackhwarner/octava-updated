...
const SuggestedCollaborators = ({
  ...
  return <div className="border border-gray-200 rounded-lg p-6 bg-white">
      <h2 className="text-2xl font-bold text-gray-900 mb-5 flex items-center">
        <Users className="w-6 h-6 mr-2 text-purple-600" />
        Suggested Collaborators
      </h2>
      
      {collaborators.length === 0 ? <div className="text-center py-8">
          ...
          {collaborators.map(profile => <Card key={profile.id} className="hover:shadow-md transition-shadow">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    <UserAvatar name={profile.name} src={profile.avatar_url} size="sm" />
                    <div>
                ...
                <div className="space-y-3">
                  ...
                  {profile.instruments && profile.instruments.length > 0 && <div>
                      <div className="flex items-center mb-2">
                        <Music className="w-4 h-4 mr-1 text-gray-500" />
                        <span className="text-sm font-medium text-gray-700">Instruments</span>
                      </div>
                      <div className="flex flex-wrap gap-1">
                        {profile.instruments.slice(0, 3).map(instrument => <Badge key={instrument} variant="outline" className="text-xs bg-purple-50 text-purple-700 border-purple-200">
                            {instrument}
                          </Badge>)}
                        {profile.instruments.length > 3 && <Badge variant="outline" className="text-xs bg-gray-50 text-gray-600">
                  ...
                  <ConnectionButton userId={profile.id} userName={profile.name} className="w-full bg-purple-600 hover:bg-purple-700 text-white" />
                </div>
              </CardContent>
            </Card>)}
        </div>}
