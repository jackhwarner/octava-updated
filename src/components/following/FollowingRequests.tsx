...
const FollowingRequests = ({
  ...
  const fetchConnectionRequests = async () => {
    ...
    try {
      ...
      const filteredIncoming = formattedIncoming.filter(request => request.name.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) || request.username.toLowerCase().includes(debouncedSearchQuery.toLowerCase()));
      const filteredOutgoing = formattedOutgoing.filter(request => request.name.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) || request.username.toLowerCase().includes(debouncedSearchQuery.toLowerCase()));
  ...
  if (loading) {
    return <div className="space-y-4">
        {[...Array(3)].map((_, i) => <div key={i} className="bg-white p-4 rounded-lg border animate-pulse">
  ...
  const renderRequestCard = (request: ConnectionRequest, isIncoming: boolean) => <div key={request.id} className="bg-white p-4 rounded-lg border hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1">
            ...
            {request.message && <p className="text-sm text-gray-600 mt-2 italic">"{request.message}"</p>}
          </div>
        </div>

        <div className="flex space-x-2">
          {isIncoming ? <>
              <Button variant="default" size="sm" onClick={() => handleAcceptRequest(request.id)} className="bg-green-600 hover:bg-green-700">
                <UserCheck className="w-4 h-4 mr-2" />
                Accept
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleDeclineRequest(request.id)} className="text-red-600 hover:text-red-700 hover:bg-red-50">
  ...
  return <div className="space-y-6 px-5 ">
      {/* Incoming Requests */}
      <Collapsible open={incomingOpen} onOpenChange={setIncomingOpen}>
        ...
        <CollapsibleContent className="space-y-4 mt-4">
          {incomingRequests.length === 0 ? <div className="text-center py-8 bg-white rounded-lg border">
            ...
            </div> : incomingRequests.map(request => renderRequestCard(request, true))}
        </CollapsibleContent>
      </Collapsible>
      ...
      <Collapsible open={outgoingOpen} onOpenChange={setOutgoingOpen}>
        ...
        <CollapsibleContent className="space-y-4 mt-4">
          {outgoingRequests.length === 0 ? <div className="text-center py-8 bg-white rounded-lg border">
            ...
            </div> : outgoingRequests.map(request => renderRequestCard(request, false))}
        </CollapsibleContent>
      </Collapsible>
    </div>;
};
